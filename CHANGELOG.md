...
- **Earnings call decision moments (agency mechanic).** When an `earnings` catalyst is scheduled to resolve next turn on a stock the player currently holds long, the NextTurn screen now surfaces a pre-commitment `EarningsDecisionCard` with three choices: **Hold** (no change), **Trim** (sell 30% defensively), or **Add** (buy 30% tactically). The auto-advance timer is suppressed while a decision is pending so the player has time to commit before the catalyst fires next turn. Trades flow through the existing `executeBuy`/`executeSell` paths so all engine guards (cash, fees, fractional shares) apply. New `getPendingEarningsDecisions(state)` helper in `catalystSystem.ts`. Closes #36.
...
- Added regression coverage that keeps the stock universe fictional, unique, and evenly distributed across sectors.
