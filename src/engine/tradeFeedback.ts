...
export interface TradeFeedbackDetail {
  label: string;
  value: string;
  tone?: FeedbackTone;
}
...
export interface TradeFeedback {
  ...
  headline: string;
  subheadline: string;
  details: TradeFeedbackDetail[];
  cashBefore: number;
  cashAfter: number;
  cashDelta: number;
  riskBefore: { level: RiskLevel; score: number };
  riskAfter: { level: RiskLevel; score: number };
  riskDelta: number;
...
function riskText(level: RiskLevel, score: number): string {
  return `${level.toUpperCase()} ${Math.round(score)}/100`;
}

function riskTone(delta: number): FeedbackTone {
...
function sectorExposurePct(state: GameState, sector: Sector): number {
  ...
  const totalExposure = Math.max(1, longValue + shortValue);
  let sectorValue = 0;
  ...
  return roundCurrency((sectorValue / totalExposure) * 100);
}
...
export function getTradeFeedback(state: GameState, stockId: string, shares: number, action: TradeAction): TradeFeedback | null {
  ...
  if (action === 'buy') {
    ...
    details.push(
      { label: 'Cash after', value: money(cashAfter) },
      { label: 'Cash impact', value: signedMoney(cashDelta), tone: 'neutral' },
      { label: 'Position after', value: positionLabel },
      { label: 'Position size', value: `${positionWeightAfter.toFixed(1)}% of net worth`, tone: positionWeightAfter >= 50 ? 'danger' : positionWeightAfter >= 25 ? 'warning' : 'neutral' },
    );
  } else if (action === 'sell') {
    ...
    details.push(
      { label: 'Realized P/L', value: signedMoney(grossPnl - transaction.fee), tone: grossPnl - transaction.fee >= 0 ? 'positive' : 'danger' },
      { label: 'Cash after', value: money(cashAfter) },
      { label: 'Cash impact', value: signedMoney(cashDelta), tone: 'positive' },
      { label: 'Position after', value: positionLabel },
    );
    ...
    details.push(
      { label: 'Margin required', value: money(marginUsed), tone: 'warning' },
      { label: 'Cash after', value: money(cashAfter) },
      { label: 'Short exposure', value: `${shortExposure.toFixed(1)}% of net worth`, tone: shortExposure >= 50 ? 'danger' : shortExposure >= 20 ? 'warning' : 'neutral' },
      { label: 'Position after', value: positionLabel },
    );
  } else {
    ...
    details.push(
      { label: 'Realized P/L', value: signedMoney(grossPnl - transaction.fee), tone: grossPnl - transaction.fee >= 0 ? 'positive' : 'danger' },
      { label: 'Cash after', value: money(cashAfter) },
      { label: 'Cash impact', value: signedMoney(cashDelta), tone: cashDelta >= 0 ? 'positive' : 'danger' },
      { label: 'Position after', value: positionLabel },
    );
  }

  details.push(
    { label: 'Sector exposure', value: `${sectorBefore.toFixed(1)}% → ${sectorAfter.toFixed(1)}%` },
    { label: 'Risk', value: `${riskText(beforeRisk.level, beforeRisk.totalScore)} → ${riskText(afterRisk.level, afterRisk.totalScore)}`, tone: riskTone(riskDelta) },
  );
  ...
  return {
    ...
    cashAfter,
    cashDelta,
    riskBefore: { level: beforeRisk.level, score: beforeRisk.totalScore },
    riskAfter: { level: afterRisk.level, score: afterRisk.totalScore },
    riskDelta,
    positionLabel,
