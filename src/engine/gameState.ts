...
export function createNewGame(
  ...
  const state: GameState = {
    saveSlot: 'auto', runId: crypto.randomUUID(), leaderboardEntryId: null, playerName: playerName || 'Trader', career: createCareerState(careerStyle, config.startingCash, now, difficulty, challengeMode), difficulty, currentTurn: 0, currentDate: startDate,
...
function getShortError(state: GameState, stockId: string, shares: number) { const config = DIFFICULTY_CONFIGS[state.difficulty]; if (state.career?.challengeMode === 'no_shorts') return 'challenge_restricted' as const; if (!config.shortEnabled) return 'short_disabled' as const; if (!isPositiveWholeNumber(shares)) return 'invalid_shares' as const; const stock = getTradeStock(state, stockId); if (!stock) return 'stock_not_found' as const; const proceeds = roundCurrency(stock.currentPrice * shares); const marginReq = roundCurrency(proceeds * config.shortMarginRequirement); const fee = calcBrokerFee(proceeds, config); return state.cash < marginReq + fee ? 'insufficient_funds' as const : null; }
...
export function calculateGrade(state: GameState): 'S' | 'A' | 'B' | 'C' | 'D' | 'F' { const ratio = getNetWorth(state) / getCareerSeasonGoal(state); if (ratio >= 3) return 'S'; if (ratio >= 1.5) return 'A'; if (ratio >= 1) return 'B'; if (ratio >= 0.75) return 'C'; if (ratio >= 0.5) return 'D'; return 'F'; }
export function checkGameOver(state: GameState): 'win' | 'lose' | 'ongoing' { if (!state.isGameOver) return 'ongoing'; return getNetWorth(state) >= getCareerSeasonGoal(state) ? 'win' : 'lose'; }
export function tradeErrorMessage(reason: string): string { const messages: Record<string, string> = { insufficient_funds: 'Not enough cash for this trade', insufficient_shares: 'Not enough shares to sell', invalid_shares: 'Share count must be a positive whole number', invalid_target_price: 'Target price must be positive', max_limit_orders_reached: 'Maximum limit orders reached', short_disabled: 'Short selling is not available', challenge_restricted: 'This career challenge blocks that trade', no_position: 'No position to cover', stock_not_found: 'Stock not found' }; return messages[reason] || 'Trade failed'; }
